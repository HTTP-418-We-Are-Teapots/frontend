...
const Geoguesser: React.FC = () => {
  ...
  const navigateToPrevention = () => navigate('/prevention/geoguessr');

  const handleValidate = () => {
    setIsValidate((prev) => !prev);
    if (isValidate && index < mapPoints.length - 1) setIndex((prev) => prev + 1);

    if (index === mapPoints.length - 1 && isValidate) {
      setGlobalPrecision(data.reduce((a, b) => a + b, 0) / data.length);
...

      {
        isFinished &&
          <div className={styles.done}>
            <p>Vous avez terminé le jeu avec une précision de { globalPrecision } % !</p>

            <button onClick={navigateToPrevention}>Prévention</button>
          </div>
      }
